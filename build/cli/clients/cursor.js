...
const locateCursorConfig = async (customPath) => {
    if (customPath) {
...
const readCursorConfig = async (path, raw) => {
    return readJsonFile(path, raw);
};
const mergeCursorEntry = (config, entry, options) => {
    return mergeIntoMap(config, entry, options, 'mcpServers');
};
const writeCursorConfigAtomic = async (path, data) => {
