...
const constitutionMap = Object.create(null);
 ...
 */
function initializeSessionConstitution(sessionId) {
...
export function updateConstitution(sessionId, rule) {
    if (!sessionId || !rule)
        return;
    ...
    if (entry.rules.length >= MAX_RULES_PER_SESSION)
        entry.rules.shift();
    entry.rules.push(rule);
    entry.updated = Date.now();
}
export function resetConstitution(sessionId, rules) {
    if (!sessionId || !Array.isArray(rules))
        return;
    constitutionMap[sessionId] = {
...
export function getConstitution(sessionId) {
