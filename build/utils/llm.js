...
export const __testing = {
    setGenAI(client) { genAI = client; },
    setOpenAIClient(client) { openaiClient = client; },
    getGenAI() { return genAI; },
    getOpenAIClient() { return openaiClient; }
};
async function callAnthropic({ model, compiledPrompt, systemPrompt, maxTokens = 1024, temperature = 0.2, }) {
    if (!model) {
    ...
    if (!response.ok) {
        ...
        if (response.status === 401 || response.status === 403) {
            throw new Error(`Anthropic authentication failed with status ${response.status}${requestSuffix}. Verify ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN.`);
        }
    ...
    const firstTextBlock = content.find((block) => block?.type === 'text' && typeof block?.text === 'string');
    if (firstTextBlock) {
