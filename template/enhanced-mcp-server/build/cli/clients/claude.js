...
export const locateClaudeConfig = async (customPath) => {
    if (customPath) {
...
export const readClaudeConfig = async (path, raw) => {
    return readJsonFile(path, raw, 'Claude config');
};
export const writeClaudeConfigAtomic = async (path, data) => {
    await writeJsonFileAtomic(path, data);
};
export const mergeMcpEntry = (config, entry, options) => {
