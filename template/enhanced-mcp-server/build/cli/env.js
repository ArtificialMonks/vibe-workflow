...
async function writeEnvFileAtomic(path, content) {
...
export async function ensureEnv(options) {
    ...
    const validateProviderKey = (key, value) => {
    ...
    const registerValue = (key, value, sourceLabel) => {
    ...
    const hydrateFrom = (key, source, label) => {
        if (key in source) {
    ...
    for (const key of targetKeys) {
        if (process.env[key]) {
            if (registerValue(key, process.env[key], 'environment variable')) {
                continue;
            }
