...
import { createRequestScopedTransport } from './utils/httpTransportWrapper.js';
import { initializeHotReload } from './utils/fileWatcher.js';
...
export const SUPPORTED_LLM_PROVIDERS = ['gemini', 'openai', 'openrouter', 'anthropic'];
export async function createMcpServer() {
    await loadHistory();
    // Initialize hot reload for constitutional rules (if enabled)
    initializeHotReload();
    const server = new Server({ name: 'vibe-check', version: getPackageVersion() }, { capabilities: { tools: {}, sampling: {} } });
    ...
    server.setRequestHandler(CallToolRequestSchema, async (req) => {
        ...
        switch (name) {
            ...
            case 'vibe_learn': {
                const missing = [];
                if (!args || typeof args.mistake !== 'string')
                ...
                const input = {
                    ...
                    type: ['mistake', 'preference', 'success'].includes(args.type)
                        ? args.type
                        : undefined,
                    sessionId: typeof args.sessionId === 'string' ? args.sessionId : undefined
                };
            ...
            case 'update_constitution': {
                ...
                if (!args || typeof args.rule !== 'string')
                    missing.push('rule');
                if (missing.length) {
            ...
            case 'reset_constitution': {
                ...
                if (!args || typeof args.sessionId !== 'string')
                    missing.push('sessionId');
                if (!args || !Array.isArray(args.rules))
                    missing.push('rules');
                if (missing.length) {
